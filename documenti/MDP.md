...
Fai il grafo delle cabine sulla geografia locale con una heat map di densità di popolazione, che dia un'idea qualitativa dei costi (qunatità di utenti in un'area). Da mettere come prima slides in una presentazione la gente può visualizzare subito di cosa sto parlando: c'è un problema, c'è una rete e ci sono delle persone e ci sono dei costi.

Puoi controllare se le coppie (stato, azione) che hanno gradienti naturali molto alti corrispondono solo ai casi di policy (quasi) deterministica?
In quel caso può essere che un gradiente normale (PG) piccolo corrisponda ad un gradiente naturale (NPG) grande, ma che questo concretamente non si traduca in nessun effetto sulla policy (perché ormai la differenza delle theta per quelle azioni è talmente grande che la probabilità di prendere quelle azioni è essenzialmente o 1 o 0).
Se ci pensi, da una parte con il NPG hai l'identità a moltiplicare Q (e quindi il gradiente ha sempre valore Q), dall'altra con il PG hai un fattore moltiplicativo davanti pi(b) (delta(a,b) - pi(a))   che se pi è deterministica è sempre pari a 0 (o si annulla un termine o l'altro).

Se il policy gradient si ferma su delle policy in parte random, quest ovuol dire che i punti stazionari sono su qualche pezzo del simplesso.

La condizione di primo grado di estremalità è gradiente = 0 se e solo se sono all'interno del simplesso. In generale l'unica cosa che si può dire è che se siamo dentro al simplesso, perché questo (?) sia un estremo dobbiamo avere che grad f = 0, se è all'interno. Se però sei sul bordo il gradiente non deve essere zero, deve solo puntare verso l'esterno in una qualche maniera. Quindi la condizione generale di estremalità è che
$$
\forall x \in X, (x - x^*) \cdot \nabla f|_{x^*} \ge 0 \, .
$$
Questa è la condizione perché $x^*$ sia un punto di minimo. Che include il caso stazionario se siamo all'interno, ma se è sul bordo la disuguaglianza diventa strettamente positivo. Questo dà la possibilità che esitano dei valori non zero se questo si è schiacciato verso il bordo. Questo è quello che stiamo cercando in maniera più astratta di verificare. In particolare, se fosse completamente deterministica vorrebbe dire che siamo finiti in un angolo/spigolo. Noi abbiamo trovato delle soluzioni parzialmente deterministiche, dunque siamo su una faccia del simplesso. Dobbiamo capire come fare questo test sui gradienti, che non è semplicemente andare a guardare se sono zeri, perché non sono quelli che contano. Quello che va a zero sempre, in questo contesto, è il gradiente della $J$ rispetto alla policy, invece noi stiamo guardando le derivate rispetto alle $\boldsymbol \theta$. Non è necessario, nel senso che è un check di consistenza interno che tutto è stato calcolato propriamente. Però come farlo in maniera sana è una domanda, se vogliamo farlo. Il gradiente rispetto alla policy rispetto al gradiente rispetto a $\pi$ ha un ulteriore derivata Jacobiana della policy rispetto ai parametri, che rintroduce un oggetto $\delta (- \pi)$ che probabilmente è quello che ammazza tutto. Ma forse non ne vale la pena: le policy sono andate a convergenza, quindi non c'è bisogno, era una domanda sbagliata controllare che i gradienti fossero zero perché non devono esserlo. A noi interessa che il rapporto fra le policy delle due azioni sia costante, la convergenza vuol dire quello: se sono zero ok, se sono due valori random devono essere fissati, quindi il gradiente delle theta delle due deve essere uguale, perché tu alla fine hai il rapporto, quindi la differenza dei due theta, se i due theta variano nello stesso modo, la differenza si mantiene stazionaria. Se sono arrivato ad una policy stazionaria le theta possono crescere con lo stesso rate e non succede nulla. Quindi basta controllare se numericamente i gradienti naturali delle theta sono identici, anche nei casi stocastici vuol dire che siamo arrivati a convergenza. Quando definiamo la policy con la nostra parametrizzazione questo induce una traiettoria, e possiamo verificare esplicitamente che il gradiente, naturale o normale,  non è mai zero, perché continua a spingere in un angolo. Questo crea un flusso nello spazio delle policy che ti spinge verso il bordo/angolo, ma questo gradiente non muore mai. Quindi chiedere la condizione di gradiente uguale a zero non ha senso. Siccome il gradiente continua ad esserci, i theta continuano a crescere, ma crescono tutti esattamente con lo stesso rate (non se è una policy deterministica, in cui un theta cresce più di tutti gli altri). Ma se si va a schiantare su un punto sul bordo, questo vuol dire che per mantenere le due probabilità di due azioni commensurate, i theta associati a quelle due azioni devono crescere esattamente con lo stesso rate. Quindi dobbiamo avere che $\theta_t - \theta_{t-1}$ è uguale per ogni azione, dunque $\nabla J \in \mathbb R$ deve essere uguale per tutte le azioni [N.d.A. la riga nella matrice grad_J deve essere tutta uguale].



✔️ C'è un paper matematico che dà delle condizioni per cui il policy gradient, pur non essendo un problema convesso, ha comunque un minimo globale unico. Però sono delle condizioni scritte in maniera abbastanza implicita. Se si ha una parametrizzazione completa, cioè non parziale, sei dentro la classe di questo problema, quindi anche senza utilizzare una parametrizzazione non lineare complessa, ma se tu hai tanti parametri quante stati-azioni, il problema non è convesso di per sé, perché è un bordello, ma ha un unico minimo globale, quindi fa quello che ti aspetti (sia vaniglia che naturale, mi pare). Non so e non credo che valga la pena di testare se in questo caso siamo in quelle condizioni lì, però senz'altro è una reference da aggiungere dal punto di vista concettuale, per dire che questo in generale è un problema non convesso, per cui facciamo dei restarts, potrebbe essere nella classe di questi, ma non abbiamo verificato. Quello che c'è di più importante, a livello concettuale, praticamente esclude che ad un certo punto ci sia sotto nascosto un altro minimo raggiungibile da un'altra condizione iniziale, che magari ha una performance ancora migliore.
...
✔️ A livello di conclusioni, non di risultati, uno dice: al di là dei risultati specifici, altre strutture di costi in questo caso possono dare risultati differenti, la differenza in performance tra le policy euristiche e quelle ottimali trovate dal PG possono essere più grandi o più piccole, dipende dal contesto, però il metodo è comunque applicabile, scalabile etc. etc.
